...
function () {
  function Customizator() {
    ...
    this.colorPinker.addEventListener('input', function (e) {
  ...
  _createClass(Customizator, [{
    ...
    value: function onScaleChange(e) {
      ...
      var recursy = function recursy(elem) {
        elem.childNodes.forEach(function (nod) {
          if (nod.nodeName === '#text' && nod.nodeValue.replace(/\s+/g, '').length > 0) {
            if (!nod.parentNode.getAttribute('data-fz')) {
              var value = window.getComputedStyle(nod.parentNode, null).fontSize;
              nod.parentNode.setAttribute('data-fz', +value.replace(/px/g, ''));
              nod.parentNode.style.fontSize = nod.parentNode.getAttribute('data-fz') * _this2.scale + 'px';
            } else {
              nod.parentNode.style.fontSize = nod.parentNode.getAttribute('data-fz') * _this2.scale + 'px';
            }
          } else {
    ...
    value: function render() {
      ...
      document.querySelector('body').append(panel); // console.log(this.btnBlock, scaleInputS, scaleInputM);
    }
  }]);
